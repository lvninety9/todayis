    ...
    <div className="min-h-screen bg-[var(--color-surface)] dark:bg-[var(--color-surface)]">
      ...
      <section className="relative overflow-hidden py-20 lg:py-32 scroll-reveal">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-gradient-to-br from-[hsl(var(--terracotta-light))] to-[hsl(var(--accent))] opacity-20 blur-3xl" />
          <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-gradient-to-br from-[hsl(var(--sage-light))] to-[hsl(var(--primary))] opacity-20 blur-3xl" />
        </div>

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-5xl lg:text-7xl font-bold bg-gradient-to-r from-[hsl(var(--primary))] via-[hsl(var(--accent))] to-[hsl(var(--terracotta-dark))] bg-clip-text text-transparent mb-6">
            소중한 순간,
            <br />
            특별한 초대장
          </h1>
          <p className="text-xl text-[var(--text-secondary)] dark:text-[var(--text-secondary)] mb-10 max-w-2xl mx-auto">
            나만의 특별한 웨딩 초대장을 만들고,
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-center text-[var(--text-primary)] dark:text-[var(--text-primary)] mb-12">
            Todayis만의 특별함
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 scroll-reveal-stagger">
            {features.map((feature, index) => (
              <GlassCard key={index} className={`p-6 text-center hover:scale-[1.02] hover:shadow-xl transition-all duration-300 scroll-reveal-up ${index === 0 ? 'lg:col-span-2 lg:row-span-2' : ''}`}>
                ...
                <h3 className={`font-semibold text-[var(--text-primary)] dark:text-[var(--text-primary)] mb-2 ${index === 0 ? 'lg:text-xl' : 'text-lg'}`}>
                  {feature.title}
                </h3>
                <p className={`text-[var(--text-secondary)] dark:text-[var(--text-secondary)] ${index === 0 ? 'lg:text-base' : 'text-sm'}`}>
                  {feature.description}
                </p>
              </GlassCard>
            ))}
      ...
      <section className="py-20 scroll-reveal">
        <GlassCard className="max-w-3xl mx-auto px-8 py-16 text-center">
          <h2 className="text-3xl font-bold text-[var(--text-primary)] dark:text-[var(--text-primary)] mb-4">
            지금 바로 시작해보세요
          </h2>
