...
- Name: template.e2e.ts >> 템플릿 라이브러리 >> 템플릿 라이브러리 페이지가 렌더링되어야 함
- Location: src/test/e2e/template.e2e.ts:233:7
...
```ts
  ...
  138 | 
  139 |   // 초대장 API
  140 |   await page.route('/api/invitations', async (route: any) => {
